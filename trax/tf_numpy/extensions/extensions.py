...
def index_update(x, idx, y):
  x = tf_np.asarray(x)
  # TODO(wangpeng): Remove these two lines once tf.tensor_strided_slice_update
  #   and tf.tensor_scatter_nd_update support broadcasting.
  y = tf_np.asarray(y, x.dtype)
  y = tf.broadcast_to(y.data, tf.shape(x[idx].data))
  return x._index_update(idx, y)  # pylint: disable=protected-access
